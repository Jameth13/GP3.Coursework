...
void RigidBody::UpdateParent()
{
	...
	btVector3 btVec3 = btTrans.getOrigin();
	btQuaternion btQuat = btTrans.getRotation();

	Transform* transform = m_entity->GetTransform();
	transform->SetPosition(glm::vec3(btVec3.x(), btVec3.y(), btVec3.z()));
	transform->SetRotation(glm::quat(btQuat.w(), btQuat.x(), btQuat.y(), btQuat.z()));
}
